...
// Keep the ORIGINAL working extraction (39% success)
...
// IMPROVED: Add numeric answer mapping
function extractAnswer(solutionText, solutionHtml = '', choices = {}) {
  ...
  // NEW: Look for numeric answer and map to choice
  const numAnswerMatch = combined.match(/(?:answer is|final answer is)[\s~]*[\\$]*([0-9,\.]+)/i);
...
async function scrapeAMC8() {
  ...
  try {
      ...
      try {
        ...
        for (const { number, url: problemUrl } of problemLinks) {
          try {
            ...
            // Extract problem data (keep original approach)
            const problemData = await page.evaluate(() => {
              ...
              // ORIGINAL: Extract choices from LaTeX (this worked at 39%)
            ...
            if (problemData) {
              ...
              const correctAnswer = extractAnswer(problemData.solutionText, problemData.solutionHtml, choices);
    ...
    // Save data
    const filename = './amc8_data_final.json';
